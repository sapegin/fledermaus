...
export function generatePage(page, config, helpers, renderers) {
	let [ templateExtension, render ] = _.pairs(renderers).shift();
	let templateFile = `${page.layout}.${templateExtension}`;
...
export function generatePages(pages, config, helpers, renderers) {
	return pages.map(page => generatePage(page, config, helpers, renderers));
}
