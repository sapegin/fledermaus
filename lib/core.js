...
export function makeContext(page, config, helpers) {
	// Combine page data and config
	let context = _.merge({}, { config }, page);

	// Bind all helpers to context
	return Object.keys(helpers).reduce((ctx, name) => {
		ctx[name] = helpers[name].bind(ctx);
		return ctx;
	}, context);
}
